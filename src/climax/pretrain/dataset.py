...
class NpyReader(IterableDataset):
    ...
    def __iter__(self):
        ...
        for idx in range(iter_start, iter_end):
            ...
            data = np.load(path)
            '''
            Here I change the total data we get.
            We will get the union of input and output variables to make sure datamodule work properly.
            '''
            yield {k: data[k] for k in (self.variables + self.out_variables)}, self.variables, self.out_variables
...
class Forecast(IterableDataset):
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            '''
            For input x we only need to return input variables.
            '''
            ...
            '''
            Make sure the slice also can work for when max_predict_range(lead_time) = 0.
            '''
            if self.max_predict_range > 0:
                # N, C, H, W.
                inputs = x[: -self.max_predict_range]
            else:
                # N, C, H, W.
                inputs = x[:]
...
class Reconstruction(IterableDataset):
    def __init__(
        self,
        dataset: NpyReader,
    ...
    def __iter__(self):
        ...
        '''
        For reconstruction error we only return the input variable as our pretrain training loss.
        Here we just ignore the output variable.
        '''

        for data, variables, out_variables in self.dataset:
            x = np.concatenate([data[k].astype(np.float32) for k in variables], axis = 1)
            x = torch.from_numpy(x)
            y = np.concatenate([data[k].astype(np.float32) for k in variables], axis = 1)
            y = torch.from_numpy(y)
...
class IndividualReconstructionDataIter(IterableDataset):
    '''
    Like ForecastDataIter but for reconstruction.
    '''
    def __init__(
            self,
            dataset,
